...
class Food {
  ...
  static deleteFood() {
    ...
    const foodName = this.parentElement.parentElement.firstElementChild.nextElementSibling.textContent;
    $.ajax({
      method: 'DELETE',
      url: `${host}/foods/${foodId}`,
    })
    .done(this.parentElement.parentElement.remove())
    .fail(Food.errorHandler)
  }
  ...
  static editFoodName() {
    ...
    const $input = $('<input class="name-edit"/>').val( $el.text() );
    ...
    $(window).on('click', function() {
      $.ajax({
        method: "PUT",
        url: `${host}/foods/${foodId}`,
        data: { food: { name: $input.val(), calories: foodCalories } }
      })
    })
  }
  ...
  static editFoodCalorie() {
    ...
    const $input = $('<input class="calorie-edit"/>').val( $el.text() );
    $el.replaceWith( $input );

    const save = function(){
      const $div = $('<div class="grid-item food-data-calorie food">').text( $input.val() );
      $input.replaceWith( $div );
    };
    ...
    $(window).on('click', function() {
      $.ajax({
        method: "PUT",
        url: `${host}/foods/${foodId}`,
        data: { food: { name: foodName, calories: $input.val() } }
      })
    })
  }
}
